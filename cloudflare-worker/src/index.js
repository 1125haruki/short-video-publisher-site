...
const VIDEO_LIST_URL = "https://open.tiktokapis.com/v2/video/list/?fields=id,title,video_description,duration,cover_image_url,like_count,comment_count,share_count,view_count,create_time";
...
async function fetchVideoList(accessToken, maxCount = 20, cursor = null) {
  ...
  const payload = await response.json().catch(() => ({}));
  assertTikTokOk(response, payload, "Video list request failed");
  return payload.data || null;
}
...
async function handleVideoList(request, env) {
  ...
  const scopeCheck = requireScope(request, env, access.session, "video.list");
  if (!scopeCheck.ok) return scopeCheck.response;

  const body = await request.json().catch(() => ({}));
  const maxCount = body.maxCount || 20;
  const cursor = body.cursor || null;

  try {
    const data = await fetchVideoList(access.session.accessToken, maxCount, cursor);
    return withCors(request, env, json({ ok: true, data }));
  } catch (err) {
    return withCors(
      request,
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (url.pathname === "/tiktok/video-list" && request.method === "POST") {
      ...
      return html(
        `<h1>TikTok OAuth Worker</h1>
         ...
         <p>Use <code>/tiktok/creator-info</code> to load creator posting settings for Direct Post.</p>
